---
layout: page
permalink: /courses/
title: Courses
description: 
nav: true
nav_order: 3
---



| Mathematics | Control & Optimization | Aerospace Engineering |
| :--- | :--- | :--- |
| Calculus I | Signal and Feedback Systems | Introduction to Flight Mechanics |
| Calculus II | Nonlinear Dynamics | Thermodynamics and Propulsion |
| Differential Equation I | Control of Nonlinear Dynamical Systems | Incompressible Fluid Mechanics |
| Differential Equation II | Differential Geometric Method in Controls | Solid Mechanics |
| Linear Algebra | Analytical & Geometric Mechanics | Aircraft Propulsion |
| Numerical Analysis | High Energy Physics and Systems* | Compressible Fluid Mechanics |
| Real Analysis | Optimization | Aerospace Structural Mechanics |
| Multivariable Calculus | Optimization for Large-Scale ML | Spaceflight Mechanics |
| Measure Theory | Random Process in Learning & Control* | Aerospace Measurements Laboratory |
| Functional Analysis | Optimal Control Systems | Control Theory |
| Partial Differential Equations | Estimation on Lie Groups | Aircraft Structures Laboratory |
| Lie Groups & Lie Algebras | Advanced Topics in Optimization & ML<sup>†</sup> | Vibrations and Structural Dynamics |
| Modern Theory of PDEs<sup>†</sup> | Online Learning and Optimization<sup>†</sup> | Aerospace Propulsion |
| Mathematical Theory of Finite Elements<sup>†</sup> | | Aerodynamics |
| | | Flight Mechanics II |
| | | Aerodynamics Laboratory |
| | | Aircraft Propulsion Laboratory |
| | | Aircraft Design |
| | | Modeling and Simulation |
| | | Navigation and Guidance |
| | | Control Systems Laboratory |

<!-- | Mathematics | Control & Optimization |
| :--- | :--- |
| Calculus I | Signal and Feedback Systems |
| Calculus II | Nonlinear Dynamics |
| Differential Equation I | Control of Nonlinear Dynamical Systems |
| Differential Equation II | Differential Geometric Method in Controls |
| Linear Algebra | Analytical & Geometric Mechanics |
| Numerical Analysis | High Energy Physics and Systems* |
| Real Analysis | Optimization |
| Multivariable Calculus | Optimization for Large-Scale ML |
| Measure Theory | Random Process in Learning & Control* |
| Functional Analysis | Optimal Control Systems |
| Partial Differential Equations | Estimation on Lie Groups |
| Lie Groups & Lie Algebras | Advanced Topics in Optimization & ML<sup>†</sup> |
| Modern Theory of PDEs<sup>†</sup> | Online Learning and Optimization<sup>†</sup> |
| Mathematical Theory of Finite Elements<sup>†</sup> | | -->

"*" indicates the highest scorer.

"<sup>†</sup>" indicates sit through in that particular course.
